...
export const hasOneOf = (targetarr, arr) => {
  ...
  return arr?targetarr.some(_ => arr.indexOf(_) > -1):arr;
}
...
export const getNewDate = ( v , noTime ) => {
  const d = v || new Date();
  const year = d.getFullYear();
  const month = getHandledValue(d.getMonth() + 1);
  const date = getHandledValue(d.getDate());
...
* 数组去重
*/
export const unique_ = function (arr,val) {
  let hash = {};
...
* 时间字符串 格式化
*/
export const getDateToStr = function (str) {
...
// 自动增加，补齐位数
export const autoNumber = function (number, i) {
